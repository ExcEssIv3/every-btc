...
import { useBtcSearch } from "../../../hooks/use-btc-search";
...
const ShowSearchButton = styled(UnstyledButton)`
  ...
  @media (hover: hover) {
    &:hover { background-color: var(--slate-400); }
  }
`;
...
function SearchWidget({
  ...
  const { searchKey, currentKey, nextKey, previousKey } = useBtcSearch({
    displayedKeys,
    virtualPosition,
  });
  ...
  }, [setVirtualPosition, currentKey, MAX_POSITION]);
  ...
  }, [searchDisplayed, cmdKey]); // eslint-disable-line react-hooks/exhaustive-deps
